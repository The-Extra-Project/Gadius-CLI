...
import {createWallet, walletStatus, mintTokens} from "./commands/contract-operations"
...
program.addHelpText("before", "CLI tool for running compute 3D geospatial compute jobs")
...
program
  ...
  program.command("create-wallet").description("this creates the user wallet to run jobs on lillypad")
  .option('-p, --private_key <key>', 'integrates the wallet to the specific account')
  .action(createWallet)


  program.command("current-wallet-status")
  .description("gets current wallet amount and status")
  .action(walletStatus)
  
  program.command("mint_token").description("demo function for transferringf some test token in order to schedule the jobs on bacalau testnet")
  ...
  .description("allows user to create compute job on bacalhau / lillypad network with given catesian points as location")
  ...
  .description("allows user to create compute job resulting rendering of the bounded polygon region")
  
  
  program.command("result job").description("allows user to create compute job result with the input as the various input parameters as output")
  ...
  program.command("get-status").description("gets the status of the current job on the lilypad network")
  .option("-j, --jobId <jobId>", "jobId to check status")
  .option("-r, --resultid <resultId>", "resultId corresponding to the stored job")


  program.command("open visualizer").description("poups the brower based rendering application in order to visualize the details")
